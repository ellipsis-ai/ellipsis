  ...
  "findBehaviorGroupsMatching" should {
    ...
    "respond with matching behaviorGroups" in new ControllerTestContextWithLoggedInUser {
      ...
      running(app) {
        ...
        val behaviorVersion = BehaviorVersion(IDs.next, behavior, behaviorGroupVersion, None, None, None, None, forcePrivateResponse = false, canBeMemoized = false, OffsetDateTime.now)
        ...
        when(dataService.managedBehaviorGroups.maybeFor(behaviorGroup)).thenReturn(Future.successful(None))
