  ...
  "createFor" should {
    ...
    "maintain saved answers" in {
      withEmptyDB(dataService, { () =>
        ...
        val behaviorVersionData = BehaviorVersionData.newUnsavedFor(group.team.id, isDataType = false, isTest = false, maybeName = None).copy(
        ...
        val secondGroupVersion = runNow(dataService.behaviorGroupVersions.createForBehaviorGroupData(group, user, groupVersionData, forceNode6 = false))
    ...
    "ensures that data type names have the same format" in {
      withEmptyDB(dataService, { () =>
        ...
        val dataTypeVersionData = BehaviorVersionData.newUnsavedFor(team.id, isDataType = true, isTest = false, maybeName = Some("_a data type"))
