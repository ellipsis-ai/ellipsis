...
class RegexMessageTriggerSpec extends MessageTriggerSpec {
  ...
  def triggerFor(pattern: String, requiresBotMention: Boolean = false, isCaseSensitive: Boolean = true): RegexMessageTrigger = {
    ...
    val group = BehaviorGroup(IDs.next, "", None, None, team, ZonedDateTime.now)
    val behavior = Behavior(IDs.next, team, Some(group), Some(versionId), None, None, ZonedDateTime.now)
    val behaviorVersion = BehaviorVersion(versionId, behavior, None, None, None, None, forcePrivateResponse = false, None, ZonedDateTime.now)
