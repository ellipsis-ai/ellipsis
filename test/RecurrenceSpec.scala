...
class RecurrenceSpec extends PlaySpec {
  ...
  "Daily" should {
    ...
    "be created with implied frequency of 1" in {
      Recurrence.maybeFromText("every day", timeZone) mustBe Some(Daily(1, Recurrence.currentAdjustedTime, timeZone))
    }

    "be created with frequency" in {
      Recurrence.maybeFromText("every 4 days", timeZone) mustBe Some(Daily(4, Recurrence.currentAdjustedTime, timeZone))
    }
  ...
  "Weekly" should {
    ...
    "be created with implied frequency of 1" in {
      Recurrence.maybeFromText("every week", timeZone) mustBe Some(Weekly(1, DateTime.now.getDayOfWeek, Recurrence.currentAdjustedTime, timeZone))
    }

    "be created with frequency" in {
      Recurrence.maybeFromText("every 2 weeks", timeZone) mustBe Some(Weekly(2, DateTime.now.getDayOfWeek, Recurrence.currentAdjustedTime, timeZone))
    }
