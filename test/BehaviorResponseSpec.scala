  ...
  "BehaviorResponse" should {
    ...
    "choose the most specific trigger" in new TestContext {
      running(app) {
        ...
        val fooParam = BehaviorParameter(IDs.next, 1, Input(IDs.next, "foo", None, TextType), version)
        val barParam = fooParam.copy(id = IDs.next, input = Input(IDs.next, "bar", None, TextType), rank = 2)
