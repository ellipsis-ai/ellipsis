...
describe('BehaviorVersion', () => {
  ...
  describe('equality', () => {
    ...
    it('finds two versions equal with different timestamps', () => {
      const version1 = BehaviorVersion.fromJson(behaviorVersionTask1).clone({ createdAt: 1450000000000 });
      const version2 = version1.clone({ createdAt: 1451000000000 });
      expect(version1.isIdenticalToVersion(version2)).toBe(true);
    });
  });

  describe('toJSON', () => {
    it('nulls out createdAt from JSON stringify for save/export', () => {
      const version = BehaviorVersion.fromJson(behaviorVersionTask1).clone({ createdAt: 1450000000000 });
