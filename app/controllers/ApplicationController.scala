  ...
  def signInWithSlack(maybeRedirectUrl: Option[String]) = UserAwareAction { implicit request =>
    val maybeResult = for {
      ...
      clientId <- configuration.getString("silhouette.slack.clientID")
        ...
        val redirectUrl = routes.SocialAuthController.authenticateSlack(maybeRedirectUrl.map(UriEncoding.encodePathSegment(_, "utf-8"))).absoluteURL(secure = request.secure)
