...
class ApplicationController @Inject() (
  ...
  def newAPIToken() = SecuredAction.async { implicit request =>
    val user = request.identity
    val action = for {
  ...
  def createAPIToken  = SecuredAction.async { implicit request =>
    val user = request.identity
      ...
      info => {
        val action = for {
          ...
          maybeToken <- teamAccess.maybeTargetTeam.map { team =>
            ...
            Redirect(routes.ApplicationController.viewAPIToken(token.id))
          }.getOrElse {
            NotFound("")
          }
  ...
  def viewAPIToken(tokenId: String) = SecuredAction.async { implicit request =>
    val user = request.identity
    val action = for {
      teamAccess <- user.teamAccessFor(None)
  ...
  def listAPITokens() = SecuredAction.async { implicit request =>
    val user = request.identity
    val action = for {
      teamAccess <- user.teamAccessFor(None)
  ...
  def revokeAPIToken = SecuredAction.async { implicit request =>
    val user = request.identity
