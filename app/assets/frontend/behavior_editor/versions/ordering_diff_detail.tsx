...
class OrderingDiffDetail extends React.PureComponent<Props> {
  ...
  orderedList(items: Array<Diffable>) {
    return items.map((ea, index) => {
      ...
      return (
        <span key={`item${index}`} className="mrs">
            <span
              className={`mhs bg-almost-black type-white phxs border-radius min-width-border-radius display-inline-block align-c type-xs`}>{number}</span>
        ...
        <div className="columns columns-elastic">
          <div className="column-group">
            <div className="column-row">
              <div className="column column-shrink bg-pink-light pvxs phs align-r"><span
                className="type-label">Before</span></div>
              <div className="column column-expand bg-pink-lightest pvxs phs">{this.getFromOrder()}</div>
            </div>
            <div className="column-row">
              <div className="column column-shrink bg-green-light pvxs phs align-r"><span
                className="type-label">After</span></div>
