...
class RequiredAWSConfigsTable(tag: Tag) extends Table[RawRequiredAWSConfig](tag, "required_aws_configs") {
  ...
  def * = (id, exportId, nameInCode, groupVersionId, maybeConfigId) <> ((RawRequiredAWSConfig.apply _).tupled, RawRequiredAWSConfig.unapply _)
}
...
class RequiredAWSConfigServiceImpl @Inject() (
  ...
  def createForAction(data: RequiredAWSConfigData, groupVersion: BehaviorGroupVersion): DBIO[RequiredAWSConfig] = {
    for {
      ...
      required <- {
        val newInstance = RequiredAWSConfig(IDs.next, data.exportId.getOrElse(IDs.next), data.nameInCode, groupVersion, maybeConfig)
