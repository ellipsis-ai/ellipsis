  ...
  def build(implicit ec: ExecutionContext): Future[Unit] = {
    ...
    for {
      _ <- if (requiredModules.isEmpty) {
        ...
        Future {
          blocking(
            Process(Seq("bash", "-c", s"cd $dirName && npm --loglevel=error init -f && npm --loglevel=error install --no-package-lock --target_platform=linux --target_libc=glibc --arch=x86_64 ${requiredModules.mkString(" ")}"), None, "HOME" -> "/tmp").!(logger)
          )
        }
      }
      _ <- Future {
